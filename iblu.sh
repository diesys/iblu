...
actualBL=$( cat /sys/class/backlight/intel_backlight/brightness )
...
if [[ $1 == "-i" ]]
	then
		...
		echo $(( $actualBL + 100 )) #> /sys/class/backlight/intel_backlight/brightness
	...
	then
		if [[ $(( $(( $actualBL - $delta )) > 0 )) ]]
			then
				# echo $(( $( cat /sys/class/backlight/intel_backlight/brightness) - 100 )) #> /sys/class/backlight/intel_backlight/brightness
				echo $(( $actualBL - $delta )) #> /sys/class/backlight/intel_backlight/brightness
		fi
